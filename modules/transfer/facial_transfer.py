...
class FacialTransfer(object):
    ...
    def run(self, content_image: str, style_image: str,
            ...
            content_size: int, style_size: int, iterations: int, **kwargs):
        """
        Start transfer.
        ...
        output_image = Variable(content_image.data, requires_grad=True)
        ...
        for i in trange(iterations):
            ...
            def closure():
                ...
                optimizer.zero_grad()
                # Calculate losses.
                output_features = self.vgg(output_image)
                total_loss = content_weight * content_loss(output_features) + style_weight * style_loss(output_features) + tv_weight * tv_loss(output_image)
                total_loss.backward()
                return total_loss

            # Optimize.
            loss = optimizer.step(closure)
            tqdm.write(str(loss.data.cpu().numpy()[0]))
